...
describe('RedFlagDetails', () => {
  ...
  const contractQualityCategory = mockProject.analysisResult!.categoryScores.find(
  ...
  test('renders category details and red flags correctly', () => {
    ...
    expect(screen.getByText(`${categoryInfo.title.toUpperCase()} ANALYSIS`)).toBeInTheDocument();
    
    // Check score display
    expect(screen.getByText(`${categoryInfo.title} Score: ${completenessCategory.score}/${completenessCategory.maxScore}`)).toBeInTheDocument();
    expect(screen.getByText(categoryInfo.description)).toBeInTheDocument();
    
    // Check red flags section title
    expect(screen.getByText(`IDENTIFIED ISSUES (${completenessCategory.redFlags.length})`)).toBeInTheDocument();
    ...
    for (const redFlag of completenessCategory.redFlags) {
      ...
      expect(screen.getByText(`-${redFlag.pointsDeducted} points`)).toBeInTheDocument();
