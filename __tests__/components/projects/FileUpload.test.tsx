...
describe('FileUpload', () => {
  ...
  const mockFile = new File(['test file content'], 'test-file.pdf', { type: 'application/pdf' });
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (projectDetailApi.uploadFile as jest.Mock).mockResolvedValue(mockUploadedFile);
  });
  ...
  test('renders the component correctly', () => {
    ...
    expect(screen.getByText('Drag and drop your files here, or click to select files')).toBeInTheDocument();
  ...
  test('uploads file when selected via button', async () => {
    ...
    await waitFor(() => {
      expect(projectDetailApi.uploadFile).toHaveBeenCalledWith(mockProjectId, mockFile);
    });
