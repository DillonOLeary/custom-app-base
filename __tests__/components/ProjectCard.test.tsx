...
describe('ProjectCard', () => {
  ...
  test('should render project card with correct information', () => {
    ...
    expect(screen.getByText('A test solar project with complete documentation')).toBeInTheDocument();
  });
  ...
  test('should render different status colors correctly', () => {
    ...
    const newProject = { ...mockProject, status: 'new' as const, score: undefined };
    const { rerender } = render(<ProjectCard project={newProject} />);
    expect(screen.getByText('New')).toBeInTheDocument();
    
    // Test pending project status
    const pendingProject = { ...mockProject, status: 'pending' as const, score: undefined };
    rerender(<ProjectCard project={pendingProject} />);
    expect(screen.getByText('Pending')).toBeInTheDocument();
    
    // Test analyzing project status
    const analyzingProject = { ...mockProject, status: 'analyzing' as const, score: undefined };
    rerender(<ProjectCard project={analyzingProject} />);
    expect(screen.getByText('Analyzing')).toBeInTheDocument();
    
    // Test failed project status
    const failedProject = { ...mockProject, status: 'failed' as const, score: undefined };
