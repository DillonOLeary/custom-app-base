...
describe('JWT Token Security', () => {
  ...
  const createMockJwt = (header: any, payload: any, signature = 'fake-signature') => {
  ...
  test('rejects tokens with invalid format', async () => {
    ...
    const result3 = await validateAndExtractTokenClaims('header.payload.signature.extra');
  ...
  test('accepts tokens with secure algorithms', async () => {
    ...
    const rs256Token = createMockJwt({ alg: 'RS256', typ: 'JWT' }, { sub: 'user-123' });
    ...
    const es256Token = createMockJwt({ alg: 'ES256', typ: 'JWT' }, { sub: 'user-123' });
